...
import ModalForStudentDetails from "modules/session/components/ModalforStudentDetails";
...
const Students = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      formatter: (cellContent, row) => {
        ...
        // const isData =
        //   row?.tbl_student_enrolments[0]?.tbl_session_time?.tbl_sessions[0]
        //     ?.ses_date;
  ...
  const getSchool = useCallback(async () => {
    const res = await dispatch(getSchoolDetail());
    setSchoolData(
      res?.payload?.data?.map(x => ({
        label: x.sc_name,
        value: x.pk_sc_id,
      })),
    );
    ...
    <div className="p-3">
      ...
      {modalForStudentDetails && (
        <ModalForStudentDetails
          ...
          subject_id={singleStudentData?.tbl_student_enrolments[0]?.tbl_enrolment_subjects?.map(
            x => x?.fk_sub_id,
          )}
