...
import LibraryItemTable from "./components/LibraryItemTable";
import {
...
import DiagnosticItemTable from "./components/DiagnosticItemTable";
import VideoItemTable from "./components/VideoItemTable";
...
const AddActivity = () => {
  ...
  const handleAddSelected = useCallback(
    (option, subject_id) => {
      if (selectedData?.length > 20) {
        ...
        if ([1, 2, 3, 4, 5, 6].includes(option.type)) {
          setSelectedData([
            ...selectedData,
  ...
  const handlePreviewAssessment = (id, topicID, yearLevelId) => {
    const fullUrl = `${process.env.REACT_APP_TUTOR_APP_URL}/${routesConstants.ADD_ACTIVITY}?is_homework=${is_homework}&sessionID=${sessionID}&studentID=${studentID}&subjectID=${subject}&typePlan=${typePlan}&yearID=${yearLevelId}&tabNumber=${selectedTab}&selectedTabEditSession=${selectedTabEditSession}&topicId=${topicID}`;
  ...
  const handlePreviewLibrary = (id, type, topicID, yearLevelId = 0) => {
    const itemType =
      +type === 1
        ...
        ? "videos"
        : "";
    const fullUrl = `${process.env.REACT_APP_TUTOR_APP_URL}/${routesConstants.ADD_ACTIVITY}?is_homework=${is_homework}&sessionID=${sessionID}&studentID=${studentID}&subjectID=${subject}&yearID=${yearLevelId}&typePlan=${typePlan}&tabNumber=${selectedTab}&selectedTabEditSession=${selectedTabEditSession}&topicId=${topicID}`;
    const url =
      +type === 6
