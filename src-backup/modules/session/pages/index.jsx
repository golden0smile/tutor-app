...
const Session = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      // #5B51FF
      formatter: (cellContent, row) => {
        // const sessionDateTime = `${row?.ses_from_date} ${row?.ses_start_time}`;
        // const currentDateTime = moment();
        let status = "";
        // const compareDate = moment(sessionDateTime, "YYYY-MM-DD HH:mm:ss");

        // if (compareDate.isAfter(currentDateTime)) {
        //   status = 1; //upcoming
        // }
        if (row?.ses_reassigned === 1) {
          status = 2; //reassigned
        } else if (row?.ses_is_completed === 1) {
          status = 4; //past or completed
        } else if (row?.ses_status === 0) {
          status = 1;
        } else if (row?.ses_status === 1) {
          status = 3; //ongoing
        }
        const customBTn =
          +status === 1
