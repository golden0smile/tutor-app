...
import AwardedCoinsSuccessModal from "../components/AwardedCoinsSuccessModal";
import {
  EndSessionModal,
  MainContentModal,
} from "../components/ModalForEndSession";
import { AddCoinSection } from "../components/ModalforAwardSession";
...
const SessionDetails = () => {
  ...
  const getValues = () => {
    ...
    setLoading(true);
    let payload = {
      sessionId: params.sessionDetailedId || query?.sessionDetailedId,
  ...
  }, [selected, params.sessionDetailedId]);
  ...
  const handleSessionStart = () => {
    // checking if there is any student with no activities
    // let studentArrayWithoutActivities =
    //   sessionData?.tbl_session_time?.tbl_student_enrolments?.filter(
    //     (item, i) =>
    //       item?.tbl_student?.Lesson?.LessonActivities?.length === 0 ||
    //       !item?.tbl_student?.Lesson,
    //   );
    // if (studentArrayWithoutActivities?.length > 0) {
    //   toast.error(
    //     `Please add at least one activity for all students to start the session`,
    //   );
    // } else {
    //   setShow(true);
    // }
  ...
  return (
    <div className={styles.sessionDetailsMainContainer}>
      ...
      <CommonModal
        ...
        // btnPrimaryClassName="rounded-sm"
