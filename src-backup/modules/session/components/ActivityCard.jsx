...
const ActivityCard = ({
  ...
  const handleQuestionStatus = (item, i) => {
    if (values?.activity_type === 5 && item?.marked_for_manual_marking === 0) {
      // return <img src={ManualMarking} alt="" />;
  ...
  return (
    <>
      ...
      nextActivityData?.TopicActivities?.length > 0 ? (
        ...
        />
      ) : [1, 2, 3, 4, 5].includes(activityType?.type) &&
        activityType?.activityId === values?.pk_lesson_activity_key &&
...
// &&activityId === values?.activity_node?.activity_node_id
