...
export const getOverlayData = (
  ...
  // debugger;
  let overLayData = [];
  let studentAttemptCount = activity_list?.filter(
    i => i?.[handleQuestionObject(subject_id)]?.StudentAnsAttempts?.length > 0,
  )?.length;
  ...
  let englishAutoMarkCount =
    ...
    activity_list?.filter(
      (item, key) =>
        ...
        item?.english_question?.english_question_parts[0]
          ?.english_question_part_automark === 0,
    )?.length;
  // : 0;
  // status=1
