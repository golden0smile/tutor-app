...
const ReScheduleEditPlan = ({
  ...
  const handleRepeat = (data = "") => {
    ...
    const activityDetails = [
      data.activity_type === 7
        ? {
            activity_node_id: data?.workSheet?.id,
            type: +data?.activity_type, // Type of activity
            topic_id: data?.workSheet?.topic_id, //parent topic id
            subject_id: data?.workSheet?.subject_id,
            is_homework: data?.marked_for_homework,
          }
        : data.activity_type === 6
        ? {
            activity_node_id: data?.activity_node?.activity_node_id,
            type: +data?.activity_type, // Type of activity
            topic_id: data?.activity_node.topic_id,
  ...
  const getDisabledStatus = () => {
    if (
      ...
      +editLessonPlan?.tbl_session?.ses_is_completed &&
      type === 0
    ) {
      return true;
    } else {
...
export default ReScheduleEditPlan;
