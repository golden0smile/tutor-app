...
const ReSchedulePlanSection = ({
  ...
  // const isDefer = props?.offlineStatus === 5 || props?.offlineStatus === 6;
  ...
  const handlePreviewFromSessionDetails = (node_id, type, subjectId) => {
    ...
    const fullUrl = `${process.env.REACT_APP_TUTOR_APP_URL}/${routesConstants.SESSION_PAGE}/${props?.session_key}?sessionDetailedId=${props?.session_key}&selectedTab=${props?.selectedTab}&isReschedule=true`;
    const url =
      +type === 6
  ...
  const HandleButtonContainer = () => {
    ...
    if (+props?.selectedTab === 1) {
      ...
      } else if (props?.status === 1 && props?.offlineStatus !== 5) {
        return (
          <Button
            onButtonClick={() =>
      ...
      } else if (props?.status === 0 && props?.offlineStatus !== 5) {
        return (
  ...
  return (
    <Fragment key={props?.id}>
      ...
      <div className={styles.sessionCardItemStatus}>
        <label
          className={
            props?.offlineStatus !== 5 && getStatus(activity_list) === 0
              ? styles.notCompleted
...
export default ReSchedulePlanSection;
